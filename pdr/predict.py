...
# 步骤1: 加载测试数据
test_data = pd.read_csv('data\\before\\Experiment 4\\slow_speed.csv')  # 加载测试数据，格式类似于训练数据
print (test_data)
...
# 步骤5: 加载已训练的模型
model = load_model('checkpoints\\four_layers_bilstm_without_data_augmentation_32.h5')  # 替换成你训练好的模型的文件名
